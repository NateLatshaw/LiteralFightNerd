...
data_path <- paste0(getwd(), '/files/input_files/')

# plot aesthetics
paired_colors4 <- brewer.pal(6, 'Paired')[c(6,5,2,1)]

# load data
full_df <- readRDS(paste0(data_path, 'round_decisions_and_stats_20210219.RDS'))
...
# establish features
rf_features <- names(df)[grepl('Diff_', names(df)) & names(df) != 'Diff_CtrlMin']
```
...
1. Create a machine learning model, JudgeAI, that utilizes publicly available round-level statistics and judging decisions to predict round scores. 
2. Develop a methodologically sound framework to properly illustrate how JudgeAI would have performed had it been implemented in the past.
...
ggplot(plot_df, aes(x = Year, y = N, color = Result_clean)) + 
  ...
  labs(x = '\nYear', 
       y = 'Number of fights\n', 
       title = 'Figure 1: UFC Fight Outcomes by Year', 
...
ggplot(plot_df, aes(x = Year, y = N, color = Judges_Round_Majority_Score)) + 
  ...
  labs(x = '\nYear', 
       y = 'Number of rounds\n', 
       title = 'Figure 2: UFC Round Scores by Year', 
       subtitle = 'Scores reported as: Red Corner Score - Blue Corner Score', 
       caption = paste0('\n-Round scores are displayed as: Red Corner Score - Blue Corner Score', 
                        '\n-When all 3 judges do not agree on a partiuclar round score, the majority score is used.'), 
       color = 'Round Score') + 
  scale_x_continuous(breaks = seq(2011, 2020, by = 1)) + 
...
kable(tbl_df, format = 'html', caption = 'Baseline Round Winner Accuracy by Year', align = 'c', booktabs = T) %>%
...
rf_mod <- ranger(formula = rf_formula,
                 ...
                 num.trees = 500)
# feature importance
feature_importance_df <- data.table(Variable = names(rf_mod$variable.importance), Importance = rf_mod$variable.importance)[order(-Importance)]
...
ggplot(plot_df, aes(x = probs_grp_midpoint, y = probs_grp_actual)) + 
  ...
  labs(x = '\nPredicted red corner round win probability', 
       ...
       title = 'Figure 3. Out-of-Sample Predicted Probability Calibration') + 
  theme_bw() + 
...
ggplot(plot_df, aes(x = Year, y = value, color = variable)) + 
  ...
  labs(x = '\nYear', 
       ...
       title = 'Figure 4: Model Performance by Year', 
...
kable(tbl_df, format = 'html', caption = 'Comparing Round Winner Predictions by Year', align = 'c', booktabs = T) %>%
  kable_styling(bootstrap_options = c('striped'), full_width = T, font_size = 24) %>% 
...
kable(importance_df, format = 'html', caption = 'Top 10 Most Important Model Features', align = 'l', booktabs = T) %>%
  kable_styling(bootstrap_options = c('striped'), full_width = T, font_size = 24) %>% 
  footnote(paste0('-Feature importance is computed on a model including all rounds in the period, as opposed to using one of the models created via time series cross validation.', 
                  '\n-Differences are computed by subtracting blue corner statistics from the corresponding red corner statistics.'))
...
The most obvious extension of JudgeAI would be aggregating the round-level predictions to produce a distribution of potential final scores. Since JudgeAI produces round score probabilities instead of just round win probabilities, it is possible to use the model's predictions to generate a complete probability distribution of every possible final score. This would allow us to account for the underlying uncertainty in the round-level scoring while identifying the most likely final score.

Overall, I think JudgeAI shows very promising results in aggregate but can still provide interesting context for each round scored. While it cannot be used to definitively say that a particular judge gave the wrong score for a particular round, it can be used to assess judges over a larger sample of fights. Since the model is calibrated to reflect the majority decision of judges and since it is largely successful in doing so, identifying any judges whose scores regularly deviate from JudgeAI's scores over a large sample of fights could be fruitful.
